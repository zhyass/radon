...
func TestSubNodePushFilter(t *testing.T) {
	...
	for _, testcase := range testcases {
		...
		joins, filters, err := parserWhereOrJoinExprs(node.(*sqlparser.Select).Where.Expr, s.getReferTables())
		assert.Nil(t, err)
...
func getExprInfo(expr sqlparser.Expr) exprInfo {
	...
	sqlparser.Walk(func(node sqlparser.SQLNode) (kontinue bool, err error) {
		...
		case *sqlparser.ColName:
			...
			if isContainKey(tableName, filter.referTables) {
				return true, nil
			}
...
func TestSubNodePushSelectExprs(t *testing.T) {
	...
	for _, testcase := range testcases {
		...
		s := MockSubNode(p, sub, log, route)
		fields, aggTyp, err := parserSelectExprs(node.(*sqlparser.Select).SelectExprs, s)
		assert.Nil(t, err)
...
func TestSubNodePushHaving(t *testing.T) {
	...
	for _, testcase := range testcases {
		...
		s := MockSubNode(p, sub, log, route)
		s.fields, _, err = parserSelectExprs(node.(*sqlparser.Select).SelectExprs, s)
		assert.Nil(t, err)
